...
现代化的SLAM系统图形控制界面，基于PyQt5实现，提供建图、导航与任务管理等核心功能。

## 特性

- 🎨 **现代暗色主题** - 简约沉稳，长时间使用不疲劳
- 📊 **实时状态监控** - 自动刷新系统状态、运行时间与性能指标
- 🧭 **建图/导航/任务** - 统一入口，结构清晰，便于扩展
- 🗂️ **地图管理** - 地图列表、详情与2D预览
- 🔔 **智能提示** - 状态变化和操作结果实时反馈
- 🏗️ **工程化架构** - 模块化设计，易于维护
...
│   │   │   ├── map_management_page.py
│   │   │   ├── navigation_management_page.py
...
- `/slam_manager/list_maps` - 获取地图列表
- `/slam_manager/process_map` - 地图处理
- `/slam_manager/start_navigation` - 启动导航
- `/slam_manager/stop_navigation` - 停止导航
- `/slam_manager/get_map_tasks` - 读取任务文件
- `/slam_manager/set_map_tasks` - 保存任务文件
